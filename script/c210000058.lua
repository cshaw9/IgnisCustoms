...
function s.e1tgt(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(s.e1fil,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil,tp)
	end
end
function s.e1evt(e,tp)
	...
	local g=Duel.SelectMatchingCard(tp,s.e1fil,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,tp)
	if g:GetCount()>0 then
		...
		if Duel.MoveToField(tc,tp,tp,LOCATION_FZONE,POS_FACEUP,true) then
			...
			e1b2:SetProperty(EFFECT_FLAG_IGNORE_IMMUNE)
			e1b2:SetRange(LOCATION_FZONE)
			e1b2:SetTargetRange(LOCATION_FZONE,LOCATION_FZONE)
			e1b2:SetValue(1)
