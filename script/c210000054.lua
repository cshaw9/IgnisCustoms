...
function s.e1evt(e,tp)
	...
	e1c:SetCode(EFFECT_CANNOT_SPECIAL_SUMMON)
	e1c:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e1c:SetTargetRange(1,0)
	e1c:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(e1c,tp)

	local e1d=Effect.CreateEffect(c)
	e1d:SetType(EFFECT_TYPE_FIELD)
	e1d:SetCode(EFFECT_CANNOT_MSET)
	e1d:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e1d:SetTargetRange(1,0)
	e1d:SetReset(RESET_PHASE+PHASE_END)
